...
```csharp
public sealed class ZPushSocket(ZSocketOptions options) : ZSocketBase(...)   // SendAsync only
public sealed class ZPullSocket(ZSocketOptions options) : ZSocketBase(...)   // wrapped by ZQueueSocket

ZSocket.CreatePush(ZSocketOptions?)            -> ZPushSocket
ZSocket.CreatePull(ZQueueSocketOptions?)       -> ZQueueSocket<ZPullSocket>
```
